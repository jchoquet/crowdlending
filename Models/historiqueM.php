...
function get_historique_prets()
{
    ...
    $query = "SELECT pret.id AS id_pret, objet.path_photo, objet.nom AS nom_objet, id_objet, username, isAccepted AS etat FROM pret
  ...
  ORDER BY id_pret DESC;";
    ...
    {
        array_push($informations_prets, array($row['id_pret'], $row['path_photo'], $row['nom_objet'], $row['id_objet'], $row['username'], $row['etat']));
    }
...
function get_historique_emprunts()
{
  ...
  ORDER BY id_pret DESC;";
    ...
    foreach ($DB->query($query) as $row)
    {
        array_push($informations_emprunts, array($row['id_pret'], $row['path_photo'], $row['nom_objet'], $row['id_objet'], $row['username'], $row['etat']));
    }
